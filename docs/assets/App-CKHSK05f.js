...
function ScrollModel({ modelData, stopDelay = 1e3 }) {
  ...
  const [scrollOverlayTop, setScrollOverlayTop] = reactExports.useState("0vh");
  reactExports.useEffect(() => {
    if (scroll.el) {
  ...
  reactExports.useEffect(() => {
    ...
    const scrollOverlayHeight = 200 * stops.length + "vh";
    let scrollOverlay = document.getElementById("scrollOverlay");
    if (!scrollOverlay) {
      scrollOverlay = document.createElement("div");
      scrollOverlay.id = "scrollOverlay";
      scrollOverlay.style.position = "absolute";
      scrollOverlay.style.top = scrollOverlayTop;
      scrollOverlay.style.left = "0";
      scrollOverlay.style.width = "100%";
      scrollOverlay.style.height = scrollOverlayHeight;
      uiOverlay.appendChild(scrollOverlay);
    }
    const root = clientExports.createRoot(scrollOverlay);
    root.render(
      /* @__PURE__ */ jsxRuntimeExports.jsx(jsxRuntimeExports.Fragment, { children: stops.map((stop, index) => /* @__PURE__ */ jsxRuntimeExports.jsx(React.Fragment, { children: stop.poiInfo && /* @__PURE__ */ jsxRuntimeExports.jsx(
        Box,
        {
          sx: {
            position: "absolute",
            top: `${stop.offset * (stops.length - 1) * 200 + 50}vh`,
            left: "1rem",
    ...
    return () => {
      root.unmount();
      scrollOverlay == null ? void 0 : scrollOverlay.remove();
    };
  }, [stops, scrollOverlayTop]);
  reactExports.useEffect(() => {
  ...
  useFrame((state, delta) => {
    ...
    setScrollOverlayTop(-(scroll.offset * 200 * (stops.length - 1)) + "vh");
    updateCamera();
    if (useDebug) {
